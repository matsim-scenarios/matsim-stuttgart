...
public class WireModalShareCalibratorIntoMATSimControlerListener implements BeforeMobsimListener {
    ...
    private static final List<String> modes = List.of(TransportMode.pt, TransportMode.car, TransportMode.ride, TransportMode.bike, TransportMode.walk);
    ...
    private static String identifyMainMode(Collection<Leg> legs) {

        legs.stream()
                .sorted(Comparator.comparingInt(leg -> modes.indexOf(leg.getMode())))
                .limit()
    ...
    public void notifyBeforeMobsim(BeforeMobsimEvent event) {
        ...
        for (Person person : event.getServices().getScenario().getPopulation().getPersons().values()) {
            ...
            for (var trip : trips) {
                var mainMode = identifyMainMode(trip.getLegsOnly());
            }


            var mainMode = TripStructureUtils.identifyMainMode(person.getSelectedPlan().getPlanElements());
            this.calibrator.updateModeUsage(person.getId(), "", event.getIteration());
